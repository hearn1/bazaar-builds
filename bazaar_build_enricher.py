...
Example:
    ...
    # Explicit paths (required when not running from a sibling bazaar_coach checkout):
    python bazaar_build_enricher.py https://bazaar-builds.net/category/builds/karnok-builds/ --hero Karnok \\
        --catalog-dir /path/to/bazaar_coach --names-file /path/to/bazaar_coach/card_cache_names.txt
"""
...
DEFAULT_TIMEOUT = 15
USER_AGENT = "BazaarCoach/BuildEnricher (+human-reviewed catalog helper)"

# Default to a sibling bazaar_coach checkout for ergonomic curator use.
_DEFAULT_TRACKER_ROOT = Path(__file__).parent.parent / "bazaar_coach"
...
def run(args) -> dict:
    ...
    known_items = load_known_items(catalog_dir, names_file)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--catalog-dir",
        type=Path,
        default=_DEFAULT_TRACKER_ROOT,
        metavar="PATH",
        help=(
            "Directory containing *_builds.json catalog files. "
            f"Defaults to {_DEFAULT_TRACKER_ROOT} (sibling bazaar_coach checkout)."
        ),
    )
    parser.add_argument(
        "--names-file",
        type=Path,
        default=_DEFAULT_TRACKER_ROOT / "card_cache_names.txt",
        metavar="PATH",
        help=(
            "Path to card_cache_names.txt generated by the coach's refresh_images.py. "
            f"Defaults to {_DEFAULT_TRACKER_ROOT / 'card_cache_names.txt'}. "
            "Never vendor a copy — point at the live coach checkout."
        ),
    )
    return parser
